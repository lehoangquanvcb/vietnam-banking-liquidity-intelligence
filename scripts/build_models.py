...
def daily_panel():
    ...
    if "fx" in d:
        d["fx"]=d.fx.ffill(limit=3);d["FX_z"]=z(np.log(d.fx).diff(5),CFG["z_window"])
    if "interbank" in d:
        d["interbank"]=d.interbank.ffill(limit=3);d["ON_z"]=z(d.interbank,CFG["z_window"])
    comps=[c for c in ["ON_z","FX_z","OMO_z"] if c in d]
...
def forecast_select(y,h=20):
    ...
    naive_pred=np.repeat(tr.iloc[-1],len(te))
    naive_rmse=float(mean_squared_error(te,naive_pred)**.5)
    naive_mae=float(mean_absolute_error(te,naive_pred))
    scored=[]
    ...
    best=min(scored,key=lambda r:r[0]) if scored else None

    # Governance rule: a candidate must beat naive benchmark. Otherwise publish the benchmark transparently.
    if best is None or best[0]>=naive_rmse:
        sigma=float(y.diff().dropna().std())
        horizons=np.arange(1,h+1)
        last=float(y.iloc[-1]);se=sigma*np.sqrt(horizons)
        out=pd.DataFrame({
            "horizon":horizons,"forecast":last,
        ...
        diag={
            ...
            "skill_vs_naive":0.0,"publishable":True,"confidence_grade":"LOW",
            "governance_note":"No ARIMA candidate beat the naive holdout benchmark; production forecast uses transparent random-walk benchmark."
        }
        if best:
            diag["best_arima"]=best[2];diag["best_arima_rmse"]=best[0]
        return out,diag
    ...
    return out,{
        "status":"OK","model":name,"nobs":len(y),"rmse":rmse,"mae":mae,"naive_rmse":naive_rmse,
        "skill_vs_naive":skill,"aic":float(final.aic),"bic":float(final.bic),
        "publishable":True,"confidence_grade":grade,
        "governance_note":"ARIMA candidate beat the naive benchmark on the holdout window."
    }
...
def main():
    ...
    ib_fc,ib_diag=forecast_select(d.interbank) if "interbank" in d else (pd.DataFrame(),{"status":"NO_INTERBANK_DATA","publishable":False})
